...
namespace BellmanFord
{
       ...
       where T : IComparable<T>
    {
        ...
        */
        ...
        public static bool BellmanFord(Graph<T> graph)
        {
            ...
            while(queue.Count > 0)
            {
                foreach (Vertex<T> vertex in graph.Vertices)
                {
                    vertex.CumulativeDistance = double.PositiveInfinity;
                    vertex.founder = null; 
                ...
                foreach (Vertex<T> vertex in graph.Vertices)
                {
                    ...
                    {
                        tentDistance = vertex.CumulativeDistance + vertex.Neighbors[i].Distance;
                        if (vertex.CumulativeDistance > tentDistance)
                        {
                ...
                foreach (Vertex<T> vertex in graph.Vertices)
                {
                    tentDistance = vertex.founder.CumulativeDistance + graph.GetEdge(vertex, vertex.founder).Distance; 
                    if (vertex.CumulativeDistance < tentDistance)
                    {
                        return true; 
                    }
